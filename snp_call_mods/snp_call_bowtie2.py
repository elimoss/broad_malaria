...
# 			assert not os.system(cmdline)
		...
		fastqFile1 = util_files.mkstempfname(prefix='realign-%s-'%rg_id, suffix='.1.fq')
		fastqFile2 = util_files.mkstempfname(prefix='realign-%s-'%rg_id, suffix='.2.fq')
		read_group = rg_full[rg_id].replace('\t','\\t')
		cmdline = "%s/samtools bam2fq %s | %s mem -p -R '%s' %s - > %s" % (
				samtoolsPath, bam_in, bwaPath, read_group, ref, samFile)
		log.info("bwa: %s" % cmdline)
		assert not os.system(cmdline)
		os.unlink(bam_in)
		
		# convert fastq to sam file
