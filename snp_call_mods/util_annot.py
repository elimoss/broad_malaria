# util_annot.py - A few methods for dealing with gene annotation from snpEff.
#
# requires python >= 2.6
#
# dpark@broadinstitute.org
# $Id: util_annot.py 7622 2013-04-12 20:08:00Z dpark $

import sqlite3, urllib, logging, re, os
import util_files
		...
		with util_files.open_or_gzopen(snpEffVcf, 'rt') as inf:
			...
			try:
				self.cur.executemany("""insert into annot (chr,pos,allele_ref,allele_alt,
					...
					row['REF'], row['ALT']] + parse_eff(row['CHROM'], row['POS'], row['INFO'])
					for row in ffp
					if row['ALT'] != '.'])
	...
	try:
		...
		for eff in effs:
			...
			if aa_chg:
				if effect.startswith('SYNON'):
					...
					assert 0, "unrecognized effect type (%s) for SNPs with coding change (%s)" % (effect, aa_chg)
			else:
				aas = ('','')
				codon = ''
		...
		if len(out)>1:
			...
			if out[0][0] == out[1][0]:
				...
				out = [[';'.join(util_files.unique([str(o[i]) for o in out])) for i in range(len(out[0]))]]
		eff = out[0][1:]
		return eff
...
class GeneDb(DbConnection):
	...
	def getByGeneId(self, gene_id, type='gene'):
		...
		chr,start,stop,strand = self.cur.fetchone()
		if type!='gene':
			if strand=='+':
				self.cur.execute("select start,stop from child where gene_id=? and type=? order by start", [gene_id,type])
			else:
				self.cur.execute("select start,stop from child where gene_id=? and type=? order by start desc", [gene_id,type])
			out = [(chr,x[0],x[1],strand) for x in self.cur]
		else:
			out = [(chr,start,stop,strand)]
		return out
