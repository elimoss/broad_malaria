...
# $Id: snp_call.py 7655 2013-04-26 18:23:42Z dpark $
...
global_tool_paths = {
	...
	'samtools':	'/idi/sabeti-data/software/samtools/samtools-0.1.18',
	'snpEff':	'/idi/sabeti-data/software/snpEff/snpEff_3_0a',
	'tabix':	'/idi/sabeti-data/software/tabix/tabix-0.2.5',
	'vcftools':	'/idi/sabeti-data/software/vcftools/vcftools_0.1.9/bin',
	'bass':		'/prodinfo/prodapps/dmsClient',
	}

def main():
	version="$Id: snp_call.py 7655 2013-04-26 18:23:42Z dpark $"[5:-2]
	description = """
...
def gatk_ug(inBams, inFasta, outVcf,
	...
	gatkPath=global_tool_paths['gatk'], tabixPath=global_tool_paths['tabix'], vcftoolsPath=global_tool_paths['vcftools'], minBQ=30):
	''' Call SNPs with GATK '''
	...
	cmdline = "java -Xmx2g -Djava.io.tmpdir=%s -jar %s -T UnifiedGenotyper --output_mode EMIT_ALL_SITES -stand_emit_conf 0.0 -stand_call_conf 0.0 %s -R %s -o %s --min_base_quality_score %d" % (
		tempfile.tempdir, gatkPath+'/GenomeAnalysisTK.jar',
		' '.join(['-I '+i for i in inBams]), inFasta, tmpFile, minBQ)
	if ploidy != 2:
...
file name as the input bam file.""",
	...
	parser.add_option("--tmpDir", dest="tmpDir", type='string',
					  help="Directory for temp files.  [default: %default]",
					  default=global_tool_paths['tmpDir'])
	parser.add_option("--minBQ", dest="minBQ", type='float',
					  help="Minimum base quality to use for SNP calling.  Capped at read MQ.  [default: %default]",
					  default=30.0)
	return parser
...
def main_gatk_ug(args, options):
	...
	gatk_ug(bamfiles, ref, tmpVcf1,
		...
		gatkPath=options.gatkPath, tabixPath=options.tabixPath, vcftoolsPath=options.vcftoolsPath, minBQ=options.minBQ)
...
def main_sen_unique_patients(args, options):
	...
	for p,runs in patients.items():
		if len(runs)>1:
			runs = [(call_rates[r],r) for r in runs]
			runs.sort()
			runs.reverse()
...
def vcf_snpEff(inVcf, genome, outVcf,
	...
	if outVcf.endswith('.gz'):
		...
		tmpVcf = outVcf
	cmdline = "cat %s | java -Xmx2g -Djava.io.tmpdir=%s -jar %s/snpEff.jar eff -c %s/snpEff.config -o vcf %s -treatAllAsProteinCoding false -noLog -ud 0 -noStats > %s" % (
		inVcf, tempfile.tempdir, snpEffPath, snpEffPath, genome, tmpVcf)
