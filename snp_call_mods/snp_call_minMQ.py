...
def gatk_ug(inBams, inFasta, outVcf,
	...
	gatkPath=global_tool_paths['gatk'], tabixPath=global_tool_paths['tabix'], vcftoolsPath=global_tool_paths['vcftools'], minMQ=30):
	''' Call SNPs with GATK '''
	...
	cmdline = "java -Xmx2g -Djava.io.tmpdir=%s -jar %s -T UnifiedGenotyper --output_mode EMIT_ALL_SITES -stand_emit_conf 0.0 -stand_call_conf 0.0 %s -R %s -o %s --min_mapping_quality_score %d" % (
		tempfile.tempdir, gatkPath+'/GenomeAnalysisTK.jar',
		' '.join(['-I '+i for i in inBams]), inFasta, tmpFile, minMQ)
	if ploidy != 2:
...
file name as the input bam file.""",
	...
	parser.add_option("--tmpDir", dest="tmpDir", type='string',
					  help="Directory for temp files.  [default: %default]",
					  default=global_tool_paths['tmpDir'])
	parser.add_option("--minMQ", dest="minMQ", type='float',
					  help="Minimum read mapping quality to use for SNP calling.  [default: %default]",
					  default=30.0)
	return parser
...
def main_gatk_ug(args, options):
	...
	gatk_ug(bamfiles, ref, tmpVcf1,
		...
		gatkPath=options.gatkPath, tabixPath=options.tabixPath, vcftoolsPath=options.vcftoolsPath, minMQ=options.minMQ)
